/*
 * Add 'cabinet' to the profile_sites.template_id allowed-values check.
 *
 * DATA SAFETY:
 * - No DELETE, TRUNCATE, or DROP TABLE.
 * - No existing column data is modified; every existing row is left untouched.
 * - Only the CHECK constraint definition changes (allowed enum widens to 20 values).
...
comment on column public.profile_sites.template_id is
  'Visual template id. One of 20 layouts.';
