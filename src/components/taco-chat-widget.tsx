...
const TACO_RESPONSES: { keywords: string[]; replies: string[] }[] = [
  ...
  },
  {
    ...
    replies: [
      ...
      'I once attended a gallery opening. The canapes were acceptable. Anyway — yes, galleries have their own tools here.',
...
interface TacoChatWidgetProps {
  /**
   * Override the default fixed positioning.
   * Defaults to "bottom-6 right-6" — pass a different Tailwind
   * position string when the widget needs to avoid overlapping UI on
   * specific layouts (e.g. admin sidebar, cookie banner).
   * Example: "bottom-6 right-6 md:bottom-10"
   */
  positionClassName?: string;
}

// Paths where the widget should not render (have their own navigation/feedback UIs).
const HIDDEN_PATH_PREFIXES = ['/admin', '/investors', '/profile/site/preview'];

export function TacoChatWidget({ positionClassName = 'bottom-6 right-6' }: TacoChatWidgetProps) {
  ...
  useEffect(() => {
    if (open && messages.length === 0) {
      ...
      setMessages([{ id: msgIdCounter++, role: 'taco', text: greeting, ts: Date.now() }]);
    }
    if (open) {
  ...
  const sendMessage = () => {
    ...
    const userMsg: Message = { id: msgIdCounter++, role: 'user', text, ts: Date.now() };
  ...
  /*
   * Single fixed anchor at the configured corner. Both the trigger button and
   * the chat panel are children so they move together when positionClassName
   * is overridden for a specific layout context.
   */
  return (
    <div className={`fixed ${positionClassName} z-50`}>
      {/* Chat panel — floats above the trigger button */}
      <div
        className={`absolute bottom-[calc(100%+1rem)] right-0 flex w-80 flex-col overflow-hidden rounded-2xl shadow-2xl transition-all duration-300 sm:w-96 ${
          open
      ...
      <button
        ...
        className={`relative flex h-14 w-14 items-center justify-center rounded-full shadow-xl ring-2 ring-pink-400/30 transition-all duration-300 hover:scale-110 focus:outline-none focus-visible:ring-4 focus-visible:ring-pink-400 ${
