  ...
  return (
    <div style={{ fontFamily: 'var(--site-font-body, ui-monospace, "SF Mono", Menlo, monospace)', color: surface.ink, background: surface.bg }}>
      <header className="border-b px-6 py-5 flex items-center justify-between" style={{ borderColor: '#ddd' }}>
        <EditableText field="display_name" value={site.display_name ?? site.name} placeholder="Your name" as="span" className="text-xs uppercase tracking-[0.15em]" />
        <nav className="flex items-center gap-4">
          {site.sections.artworks && site.artworks.length > 0 && <a href="#works" className="text-[10px] uppercase tracking-widest hover:opacity-50" style={{ color: accentColor }}>Index</a>}
          {site.sections.contact && <a href="#contact" className="text-[10px] uppercase tracking-widest hover:opacity-50" style={{ color: accentColor }}>Contact</a>}
        ...
        slots={{
          bio: site.bio ? (
            <section className="max-w-4xl mx-auto px-6 py-8 border-b" style={{ borderColor: '#eee', fontFamily: 'system-ui, sans-serif' }}>
              <EditableText field="bio" value={site.bio} placeholder="Write a short bio…" as="p" className="text-xs leading-relaxed" style={{ color: '#666' }} />
            </section>
          ) : undefined,
          ...
          press: site.press.length > 0 ? (
            <section className="max-w-4xl mx-auto px-6 py-10 border-t" style={{ borderColor: '#eee', fontFamily: 'system-ui, sans-serif' }}>
              <p className="text-[10px] uppercase tracking-widest mb-6" style={{ color: accentColor }}>Press</p>
              <div className="space-y-3">
                {site.press.map((item, i) => (
                  <a key={i} href={item.url} target="_blank" rel="noreferrer" className="block hover:opacity-60 transition-opacity">
                    <p className="text-xs">{item.title}</p>
                    {item.publication_name && <p className="text-[10px] mt-0.5" style={{ color: '#aaa' }}>{item.publication_name}</p>}
                  </a>
                ))}
              </div>
            </section>
