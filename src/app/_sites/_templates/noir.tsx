  ...
  return (
    <div style={{ fontFamily: 'var(--site-font-body, system-ui, -apple-system, sans-serif)', color: '#fff', background: '#000' }}>
      <header className="px-6 md:px-12 py-10 flex items-end justify-between border-b border-white/10">
        <div>
          <h1 className="text-sm uppercase tracking-[0.35em] text-white/70">
            <EditableText field="display_name" value={site.display_name ?? site.name} placeholder="Your name" as="span" />
          </h1>
          <EditableText field="tagline" value={site.tagline} placeholder="Your tagline…" as="p" className="text-xs mt-2 text-white/35" />
        </div>
        <nav className="flex items-center gap-5">
          {site.sections.artworks && site.artworks.length > 0 && <a href="#works" className="text-[10px] uppercase tracking-widest text-white/40 hover:text-white/70 transition-colors">Works</a>}
          {site.sections.contact && <a href="#contact" className="text-[10px] uppercase tracking-widest text-white/40 hover:text-white/70 transition-colors">Contact</a>}
        ...
        slots={{
          bio: site.bio ? (
            <section className="max-w-xl mx-auto px-6 py-14 text-center">
              <EditableText field="bio" value={site.bio} placeholder="Write a short bio…" as="p" className="text-sm leading-relaxed text-white/45" />
            </section>
          ) : undefined,
          ...
          contact: (
            <footer id="contact" className="border-t border-white/10 px-6 py-16 text-center">
              <p className="text-[10px] uppercase tracking-[0.3em] text-white/30 mb-8">
                {site.display_name ?? site.name} · Photographer
              </p>
