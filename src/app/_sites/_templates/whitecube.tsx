    ...
    <div style={{ fontFamily: 'Georgia, "Times New Roman", serif', color: surface.ink, background: surface.bg }}>
        ...
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-6">
            {site.sections.artworks && site.artworks.length > 0 && (
              <a href="#works" className="text-[10px] uppercase tracking-[0.25em] hover:opacity-50 transition-opacity" style={{ color: '#999' }}>
                Works
              </a>
            )}
            {site.sections.exhibitions && site.exhibitions.length > 0 && (
              <a href="#exhibitions" className="text-[10px] uppercase tracking-[0.25em] hover:opacity-50 transition-opacity" style={{ color: '#999' }}>
                Exhibitions
              </a>
            )}
            {site.sections.contact && (
              <a href="#contact" className="text-[10px] uppercase tracking-[0.25em] hover:opacity-50 transition-opacity" style={{ color: '#999' }}>
                Contact
              </a>
            )}
      ...
      {site.sections.bio && site.bio && (
        <section className="max-w-xl mx-auto px-6 py-16 text-center">
          <p className="text-sm leading-relaxed" style={{ color: '#666' }}>{site.bio}</p>
        </section>
      )}
      ...
      {site.sections.artworks && site.artworks.length > 0 && (
        <section id="works">
          {site.artworks.map((artwork, i) => (
            <article
              key={artwork.id}
              className="min-h-[70vh] flex flex-col items-center justify-center px-6 py-20 border-t"
            ...
            >
              <Link href={`/works/${artwork.id}`} className="group block w-full max-w-2xl">
                ...
                <div className="mt-8 text-center max-w-md mx-auto">
                  <p className="text-[10px] uppercase tracking-[0.2em] mb-2" style={{ color: accentColor }}>
                    {String(i + 1).padStart(2, '0')}
                  </p>
                  <h2 className="text-sm font-normal italic">{artwork.title}</h2>
                  {artwork.artist_name && (
                    <p className="text-xs mt-1" style={{ color: '#888' }}>{artwork.artist_name}</p>
                  )}
                  <p className="text-[10px] uppercase tracking-[0.15em] mt-3" style={{ color: '#aaa' }}>
