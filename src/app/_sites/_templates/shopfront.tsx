  ...
  return (
    <div style={{ fontFamily: bodyFont, background: surface.bg, color: surface.ink }}>
      ...
      <nav
        ...
        style={{ background: surface.bg, borderColor: borderColor(site.surface_color) }}
      >
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-3.5 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5 text-sm">
            {site.sections.artworks && site.artworks.length > 0 && (
              <a href="#shop" style={{ color: accentColor }} className="font-medium hover:opacity-70 transition-opacity">Shop</a>
            )}
            {site.sections.exhibitions && site.exhibitions.length > 0 && (
              <a href="#exhibitions" style={{ color: mutedText(site.surface_color, site.theme.text_color) }} className="hover:opacity-70 transition-opacity">Shows</a>
            )}
            {site.sections.press && site.press.length > 0 && (
              <a href="#press" style={{ color: mutedText(site.surface_color, site.theme.text_color) }} className="hover:opacity-70 transition-opacity">Press</a>
            )}
            {site.sections.contact && (
              <a href="#contact" style={{ color: mutedText(site.surface_color, site.theme.text_color) }} className="hover:opacity-70 transition-opacity">Contact</a>
            )}
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold mb-3" style={{ fontFamily: headingFont }}>
              {site.tagline ?? `Welcome to ${site.display_name ?? site.name}`}
            </h1>
            {site.sections.bio && site.bio && (
              <p className="text-base leading-relaxed max-w-xl" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>{site.bio}</p>
            )}
          </div>
          {site.location && (
            <p className="text-sm flex-shrink-0" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>{site.location}</p>
          )}
        </div>
      </section>
      ...
      {site.sections.artworks && site.artworks.length > 0 && (
        <section id="shop" className="max-w-6xl mx-auto px-4 md:px-8 py-10">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>
              {site.artworks.length} {site.artworks.length === 1 ? 'work' : 'works'}
            </span>
          </div>
            ...
            {site.artworks.map((artwork) => (
              ...
              >
                <div className="relative aspect-square overflow-hidden" style={{ background: `${surface.ink}06` }}>
                  ...
                  {artwork.sold_at && (
                    <span
                      className="absolute top-2 right-2 text-[10px] uppercase tracking-widest px-2 py-0.5 rounded-full"
                      style={{ background: surface.ink, color: surface.bg, fontFamily: bodyFont }}
                    >
                      Sold
                    </span>
                  )}
                  {!artwork.sold_at && artwork.for_sale && (
                    <span
                      className="absolute top-2 right-2 text-[10px] uppercase tracking-widest px-2 py-0.5 rounded-full"
                      style={{ background: accentColor, color: surface.bg, fontFamily: bodyFont }}
                    >
                      For sale
                    </span>
                  )}
                ...
                <div className="p-3">
                  ...
                  {artwork.artist_name && (
                    <p className="text-xs mb-1" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>{artwork.artist_name}</p>
                  )}
                  {artwork.sold_at ? (
