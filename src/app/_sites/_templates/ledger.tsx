...
export function LedgerTemplate({ site }: { site: SiteData }) {
  ...
  return (
    <div style={{ fontFamily: bodyFont, background: surface.bg, color: surface.ink, minHeight: '100svh' }}>
      ...
      {/* ── HEADER ── */}
      <header className="flex items-center justify-between gap-4 px-6 md:px-12 py-5 border-b" style={{ borderColor: borderColor(site.surface_color) }}>
        <div>
          {site.logo_image_url ? (
            ...
            <h1 className="text-sm font-semibold tracking-tight" style={{ fontFamily: headingFont }}>
      ...
      <div className="flex">
        <main className="flex-1 min-w-0">
          ...
          {site.sections.bio && site.bio && (
            <section className="border-b px-6 md:px-12 py-8 max-w-xl" style={{ borderColor: borderColor(site.surface_color) }}>
              <EditableText field="bio" value={site.bio} placeholder="Write a short bio…" as="p" className="text-sm leading-relaxed" />
            </section>
          )}
        ...
        {hoveredArtwork?.image_url && (
          <div className="hidden md:block fixed pointer-events-none" style={{ right: '2rem', top: '50%', transform: 'translateY(-50%)', width: '280px', zIndex: 50 }}>
            <div className="relative aspect-square rounded overflow-hidden shadow-2xl">
              <Image src={hoveredArtwork.image_url} alt={hoveredArtwork.title} fill className="object-cover" unoptimized />
            </div>
