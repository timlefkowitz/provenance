    ...
    <div style={{ fontFamily: bodyFont, background: surface.bg, color: surface.ink }}>
      ...
      <header className="border-b px-4 md:px-8 py-4 flex items-center justify-between gap-4" style={{ borderColor: borderColor(site.surface_color) }}>
        <div>
          {site.logo_image_url ? (
            ...
            <h1 className="font-semibold text-sm" style={{ fontFamily: headingFont }}>
      ...
      {site.sections.bio && site.bio && (
        <section className="border-b px-4 md:px-8 py-8" style={{ borderColor: borderColor(site.surface_color) }}>
          <EditableText field="bio" value={site.bio} placeholder="Write a short bio…" as="p" className="text-sm leading-relaxed max-w-2xl" style={{ color: surface.ink }} />
        </section>
      )}
          ...
          <div className="flex items-center min-w-max px-4 md:px-8">
            {years.map((yr) => (
              <button
                ...
                style={{
                  color: activeYear === yr ? accentColor : mutedText(site.surface_color, site.theme.text_color),
                  borderBottom: activeYear === yr ? `2px solid ${accentColor}` : '2px solid transparent',
                  marginBottom: '-1px',
                }}
              >
                {yr}
              ...
              {activeWorks.map((artwork) => (
                <Link key={artwork.id} href={`/works/${artwork.id}`} className="group block relative aspect-square overflow-hidden" style={{ background: `${surface.ink}06` }}>
                  {artwork.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center p-1"><span className="text-[8px] text-center leading-tight" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>{artwork.title}</span></div>
                  )}
