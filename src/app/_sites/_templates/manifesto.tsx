...
export function ManifestoTemplate({ site }: { site: SiteData }) {
  ...
  return (
    <div style={{ fontFamily: 'var(--site-font-body, system-ui, sans-serif)', background: surface.bg, color: surface.ink, minHeight: '100svh' }}>

      {/* ── HERO ── */}
      <section style={{ borderBottom: `1px solid ${accentColor}22` }} className="min-h-[80svh] flex flex-col justify-between px-6 md:px-16 py-10 md:py-20">
        ...
        <div className="mt-auto pt-20">
          <p className="text-5xl md:text-7xl lg:text-8xl font-bold leading-none tracking-tight mb-8 max-w-5xl" style={{ fontFamily: 'var(--site-font-heading, system-ui, sans-serif)', color: accentColor }}>
            <EditableText field="tagline" value={site.tagline ?? site.display_name ?? site.name} placeholder="Your tagline…" as="span" />
          </p>
          {site.sections.bio && site.bio && (
            <EditableText
              ...
              style={{ color: mutedText(site.surface_color, site.theme.text_color) }}
            />
          )}
        </div>
      </section>
        ...
        slots={{
              ...
              {site.artworks.slice(0, 12).map((artwork, i) => (
                <SiteArtworkLink key={artwork.id} artwork={artwork} className="group flex flex-col md:flex-row border-t" style={{ borderColor: `${surface.ink}14`, flexDirection: i % 2 === 0 ? 'row' : 'row-reverse' }}>
                  ...
                  <div className="md:w-1/2 flex flex-col justify-center px-6 md:px-16 py-10 md:py-16 gap-4">
                    <h3 className="text-2xl md:text-3xl font-bold leading-tight transition-opacity group-hover:opacity-70" style={{ fontFamily: 'var(--site-font-heading, system-ui, sans-serif)', color: surface.ink }}>{artwork.title}</h3>
                    {artwork.artist_name && <p className="text-sm" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>{artwork.artist_name}</p>}
                ...
                {site.exhibitions.map((ex) => (
                  <Link key={ex.id} href={`/exhibitions/${ex.id}`} className="group block border-t pt-5 hover:opacity-70 transition-opacity" style={{ borderColor: `${surface.ink}20` }}>
                    ...
                    <p className="text-xs" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>
                      {formatDate(ex.start_date)}{ex.end_date ? ` – ${formatDate(ex.end_date)}` : ''}{ex.location ? ` · ${ex.location}` : ''}
                    </p>
                  </Link>
                ))}
              </div>
          ...
          press: site.press.length > 0 ? (
            <section id="press" className="border-t px-6 md:px-16 py-16" style={{ borderColor: `${surface.ink}14` }}>
              ...
              <div className="max-w-xl"><SitePressList press={site.press} /></div>
            </section>
          ) : undefined,
