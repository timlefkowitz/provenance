...
export function BillboardTemplate({ site }: { site: SiteData }) {
  ...
  return (
    <div style={{ fontFamily: 'var(--site-font-body, system-ui, sans-serif)', background: surface.bg, color: surface.ink }}>
      ...
      {/* ── HERO BLOCK ── */}
      <section style={{ background: accentColor, color: surface.bg }} className="px-6 md:px-16 pt-10 pb-16">
        <header className="flex items-center justify-between mb-16">
          {site.logo_image_url ? (
            ...
            <EditableText field="display_name" value={site.display_name ?? site.name} placeholder="Your name" as="span" className="text-sm uppercase tracking-[0.2em] opacity-80" style={{ fontFamily: headingFont }} />
          )}
        ...
        <h1 className="text-6xl md:text-8xl lg:text-[10rem] font-black uppercase leading-none tracking-tighter" style={{ fontFamily: headingFont, color: surface.bg }}>
        ...
        <EditableText
          ...
          as="p"
          className="mt-6 text-base md:text-xl max-w-xl opacity-80"
          style={{ color: surface.bg }}
        />
        {site.cta && (
          <div className="mt-8">
        ...
        slots={{
          bio: site.bio ? (
            <section className="px-6 md:px-16 py-12 border-b" style={{ borderColor: `${surface.ink}14` }}>
              <EditableText field="bio" value={site.bio} placeholder="Write a short bio…" as="p" className="text-base md:text-xl max-w-2xl leading-relaxed" />
            </section>
          ) : undefined,
              ...
              <div className="grid grid-cols-2 gap-0.5">
                {site.artworks.slice(0, 12).map((artwork) => (
                  <SiteArtworkLink key={artwork.id} artwork={artwork} className="group block relative overflow-hidden bg-gray-50" style={{ aspectRatio: '4/3' }}>
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-4 opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: 'linear-gradient(transparent 40%, rgba(0,0,0,0.75))' }}>
          ...
          exhibitions: site.exhibitions.length > 0 ? (
            <section id="shows" className="px-6 md:px-16 py-14 border-t" style={{ borderColor: `${surface.ink}14`, background: `${accentColor}08` }}>
              <h2 className="text-4xl md:text-6xl font-black uppercase leading-none tracking-tighter mb-10" style={{ fontFamily: headingFont, color: accentColor }}>Shows</h2>
              <div className="space-y-0">
                {site.exhibitions.map((ex, i) => (
                  <Link key={ex.id} href={`/exhibitions/${ex.id}`} className="group flex items-baseline justify-between gap-6 py-5 border-b hover:opacity-70 transition-opacity" style={{ borderColor: `${surface.ink}14` }}>
                    ...
                    <span className="text-xs text-right" style={{ color: mutedText(site.surface_color, site.theme.text_color) }}>{formatDate(ex.start_date)}{ex.location ? ` · ${ex.location}` : ''}</span>
                  </Link>
                ))}
              </div>
            </section>
          ...
          press: site.press.length > 0 ? (
            <section id="press" className="px-6 md:px-16 py-14 border-t" style={{ borderColor: `${surface.ink}14` }}>
              <h2 className="text-4xl md:text-6xl font-black uppercase leading-none tracking-tighter mb-10" style={{ fontFamily: headingFont, color: accentColor }}>The Latest</h2>
              <div className="grid md:grid-cols-3 gap-4">
                {site.press.slice(0, 6).map((item, i) => (
                  <a key={i} href={item.url} target="_blank" rel="noreferrer" className="group block border-t pt-4 hover:opacity-70 transition-opacity" style={{ borderColor: accentColor }}>
                    {item.publication_name && <p className="text-[10px] uppercase tracking-widest mb-2" style={{ color: accentColor }}>{item.publication_name}</p>}
          ...
          contact: (
            <section id="contact" className="border-t px-6 md:px-16 py-14" style={{ borderColor: `${surface.ink}14` }}>
              <h2 className="text-4xl md:text-6xl font-black uppercase leading-none tracking-tighter mb-10" style={{ fontFamily: headingFont, color: accentColor }}>Contact</h2>
