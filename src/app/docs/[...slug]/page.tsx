...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!entry || entry.group !== 'guide') {
    return { title: 'Documentation | Provenance' };
  }
  return {
    title: `${entry.title} | Provenance Docs`,
        ...
        <div className="min-w-0 flex-1">
          <header className={docsPageHeader}>
            <p className="mb-2 font-mono text-[11px] text-[#1793d1]/70">
              <span className="text-[#67d4ff]">$</span> provenance-docs — user guide
            </p>
